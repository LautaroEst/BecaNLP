...
def detect_lang_spacy_es(ds,df):
...
def detect_lang_spacy_pt(ds,df):
...
def detect_lang_langdetect(ds,df):
...
def detect_lang_fasttext(ds,df):
...
def generate_masks():
	...
	df_esp = pd.read_csv(root_path + 'reviews_esp_cleaned_chars.csv')
	df_por = pd.read_csv(root_path + 'reviews_por_cleaned_chars.csv')
	df = pd.concat([df_esp,df_por],ignore_index=True)
	ds_text = (df['review_content'] + ' ' + df['review_title']).astype(np.str)

	tic = time.time()
	mask_spacy_es = detect_lang_spacy_es(ds_text,df)
	toc = time.time()
	print('Spacy-es:',toc-tic)
	mask_spacy_es.to_csv(root_path + 'mask_spacy_es.csv',index=False)

	tic = time.time()
	mask_spacy_pt = detect_lang_spacy_pt(ds_text,df)
	toc = time.time()
	print('Spacy-pt:',toc-tic)
	mask_spacy_pt.to_csv(root_path + 'mask_spacy_pt.csv',index=False)

	tic = time.time()
	mask_langdetect = detect_lang_langdetect(ds_text,df)
	toc = time.time()
	print('langdetect:',toc-tic)
	mask_langdetect.to_csv(root_path + 'mask_langdetect.csv',index=False)

	tic = time.time()
	mask_langid = detect_lang_langid(ds_text,df)
	toc = time.time()
