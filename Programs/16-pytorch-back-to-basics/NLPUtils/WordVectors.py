...
class Vocabulary(object):
    ...
    def __init__(self, tokens=[], frequencies=[]):
        ...
        for tk, fq in zip(tokens,frequencies):
            index = len(self._token_to_idx)
            self._token_to_idx[tk] = index
            self._idx_to_token[index] = tk
            self._idx_to_freq[index] = fq
...
#         self.context_indeces = torch.from_numpy(self.context_indeces)
    ...
    def __init__(self, corpus, window_size=2, cutoff_freq=0):
        ...
        counts = np.array([[doc.count(word) for doc in corpus] for word in corpus_words]).sum(axis=1).tolist()
        self.vocabulary = Vocabulary(corpus_words,counts)
