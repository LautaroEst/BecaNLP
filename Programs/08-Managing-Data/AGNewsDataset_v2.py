...
class AGNewsDataset(object):
    ...
    def __init__(self, root, train=True, use_test_tokens=True, cutoff_freq=0):
        ...
        self.preprocessed_train_path = '{}pp_train_cut_{}.csv'.format(self.root,max(0,cutoff_freq))
        self.preprocessed_test_path = '{}pp_test_cut_{}.csv'.format(self.root,max(0,cutoff_freq))
        if not (os.path.exists(self.preprocessed_train_path) and os.path.exists(self.preprocessed_test_path)):
            ...
            self.vocabulary = self._preprocess_data(use_test_tokens=use_test_tokens,cutoff_freq=cutoff_freq)
        else:
            print('Archivos train.csv y test.csv encontrados y preprocesados.')
            self.vocabulary = self._get_vocabulary(use_test_tokens=use_test_tokens,cutoff_freq=cutoff_freq)
    ...
    def _get_vocabulary(self, use_test_tokens=False, cutoff_freq=None):
        ...
        ##### TODO #####
        for filename in [self.preprocessed_train_path, self.preprocessed_test_path]:
            df = pd.read_csv(filename)
            with open(filename, 'w') as pp_f:
                pp_f.write('Title,Class label\n')
                for index, row in self._data.iterrows():
                    title = self._string_to_tokens(row['Title'])
                    new_title = ''
