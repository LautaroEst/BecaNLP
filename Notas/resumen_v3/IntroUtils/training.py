...
class Trainer(object):
    ...
    def __init__(self, model, train_dataset, val_dataset=None, test_dataset=None, batch_size=128, device='cpu'):
        ...
        self.train_dataloader = DataLoader(train_dataset, batch_size=batch_size, sampler=my_sampler(samples_idx))
        self.batch_len = len(self.train_dataloader)
        
        if val_dataset is not None:
            samples_idx = torch.randperm(len(val_dataset))
            self.val_dataloader = DataLoader(val_dataset, batch_size=batch_size, sampler=my_sampler(samples_idx))
        if test_dataset is not None:
            samples_idx = torch.randperm(len(test_dataset))
            self.test_dataloader = DataLoader(test_dataset, batch_size=batch_size, sampler=my_sampler(samples_idx))
            ...
            for e in range(epochs):
                for t, (x,y) in enumerate(self.train_dataloader):
                    ...
                    if (e * self.batch_len + t) % sample_loss_every == 0:
                        ...
                        print('Epoch: {}, Batch number: {}, Loss: {:.4f}'.format(e+1, t,loss.item()))#,acc))
