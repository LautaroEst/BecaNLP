...
def glove(df, n=100, xmax=100, alpha=0.75, max_iter=100, eta=0.05,
    ...
    m = X.shape[0]
    # Parameters:
    W = cs224uvsmutils.randmatrix(m, n)  # Word weights.
    C = cs224uvsmutils.randmatrix(m, n)  # Context weights.
    B = cs224uvsmutils.randmatrix(2, m)  # Word and context biases.
    # Precomputable GloVe values:
    X_log = cs224uvsmutils.log_of_array_ignoring_zeros(X)
